...
const nodeExecCacheDir = 'node-executables';
...
export async function downloadAndExtractNode(options: DownloadOptions): Promise<void> {
  ...
  const cachedDownloadPath = path.join(nodeExecCacheDir, `node-v${options.version}-${platform}`);
  if (fs.existsSync(cachedDownloadPath)) {
    fs.copyFileSync(cachedDownloadPath, options.destFile);
    console.log(`Node.js executable for ${platform} has been copied from cached download to ${options.destFile}.`);
    return;
  }
  ...
  try {
    ...
    if (!fs.existsSync(nodeExecCacheDir)) {
      fs.mkdirSync(nodeExecCacheDir);
    }
    fs.copyFileSync(nodeBinary, cachedDownloadPath);
