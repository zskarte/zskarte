...
// node_modules/udsv/dist/uDSV.mjs
var comma = ",";
var quote = '"';
var tab = "	";
var pipe = "|";
...
function genToTypedRows(cols, objs = false, deep = false) {
  ...
  if (objs && deep) {
    ...
    if (!objs && cols.every((c) => c.type === T_STRING)) buf = "r";
    else {
      buf = objs ? "{" : "[";
      cols.forEach((col, ci) => {
...
function initParser(schema, chunkSize) {
  ...
  const stringObjs = gen(initRows, addRows, () => {
    _toObjsS ??= genToTypedRows(cols.map((col) => __spreadProps(__spreadValues({}, col), {
      type: "s",
      repl: __spreadProps(__spreadValues({}, col.repl), {
        empty: void 0
      })
    })), true, false);
    return _toObjsS;
