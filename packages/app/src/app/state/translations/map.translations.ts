/**
 * ========================================
 * 🗺️ MAP & SPATIAL TRANSLATIONS
 * ========================================
 * 
 * This module contains translation keys specifically for map functionality, drawing tools,
 * and spatial operations in the ZSKarte disaster management application.
 * 
 * 🎯 SCOPE - This file should contain translations for:
 * 
 * ✅ Map Navigation & Controls:
 *    - Zoom operations (zoomIn, zoomOut, zoomToScale)
 *    - Map positioning (myLocation, coordinates, defineCoordinates)
 *    - Scale management (scale, scaleToUse)
 * 
 * ✅ Base Maps & Layers:
 *    - Base map providers (openStreetMap, geoAdminSwissImage, noBaseMap)
 *    - Layer management (layers, availableLayers, currentMap, addNewLayer)
 *    - Layer configuration (layerSource, layerSourceUrl, mapLayerType)
 * 
 * ✅ Drawing Tools & Features:
 *    - Drawing shapes (polygon, point, circle, line, freehand)
 *    - Drawing modes and states (drawMode, expertView, defaultView)
 *    - Feature management (selectedFeature, removeFeatureFromMapConfirm)
 * 
 * ✅ Map Data Operations:
 *    - Import/export functionality (importMap, exportSession, downloadCurrentDrawing)
 *    - Map state management (newMap, loadMap, emptyMap, currentDrawing)
 *    - Protocol export (protocolExport, csv* keys for map data export)
 * 
 * ✅ Spatial Analysis & Filtering:
 *    - Element visibility (showAllElements, hideAllElements, generalFilters)
 *    - Symbol management (drawnSymbols, highlightAll, zoomToAll, reset)
 *    - Search and location (findPlace, csvLocation, csvCentroid)
 * 
 * ❌ DO NOT add to this file:
 *    - General UI actions used across multiple domains
 *    - Journal-specific functionality
 *    - Help text or documentation content
 *    - Generic form controls (use core translations instead)
 * 
 * 📝 ADDING NEW KEYS:
 * When adding map-related translations, follow this format:
 * 
 * yourMapKey: {
 *   de: 'German translation',
 *   en: 'English translation',
 *   fr: 'French translation',
 * },
 * 
 * Current size: ~316 lines covering map interface, drawing tools, and spatial functionality.
 * ========================================
 */
export const MAP_TRANSLATIONS = {
  mapDataOvertake: {
    de: 'Kartendaten übernehmen',
    en: 'Overtake map data',
    fr: 'Appliquer les données de la carte',
  },
  newMap: {
    de: 'Neue Karte erstellen',
    en: 'Create new map',
    fr: 'Créer une nouvelle carte',
  },
  loadMap: {
    de: 'Bestehende Karte laden',
    en: 'Load existing map',
    fr: 'Charger une carte existante',
  },
  importMap: {
    de: 'Karte importieren',
    en: 'Import map',
    fr: 'Importer carte',
  },
  importMapConflict: {
    de: 'Die zu importierende Karte existiert bereits. Möchten Sie sie ersetzen? Ansonsten wird eine Kopie angelegt.',
    en: 'The map to be imported already exists. Do you want to replace it? If not, a copy will be created.',
    fr: 'La carte à importer existe déjà. Souhaitez-vous la remplacer ? Sinon, une copie est créée.',
  },
  downloadCurrentDrawing: {
    de: 'Aktuelle Zeichnung herunterladen',
    en: 'Download the current drawing',
    fr: 'Télécharger le dessin actuel',
  },
  exportSession: {
    de: 'Ereignis exportieren',
    en: 'Export event',
    fr: 'Exporter événement',
  },
  findPlace: {
    de: 'Ort finden',
    en: 'Find a place',
    fr: 'Trouver emplacement',
  },
  drawLayer: {
    de: 'Zeichnungsebene',
    en: 'Drawing layer',
    fr: 'Couche de dessin',
  },
  layers: {
    de: 'Ebenen',
    en: 'Layers',
    fr: 'Couches cartographiques',
  },
  availableLayers: {
    de: 'Verfügbare Ebenen',
    en: 'Available layers',
    fr: 'Couches cartographiques disponibles',
  },
  currentMap: {
    de: 'Basiskarten',
    en: 'Base map',
    fr: 'Carte de base',
  },
  otherMaps: {
    de: 'Andere Karten',
    en: 'Other map',
    fr: 'Autres cartes',
  },
  emptyMap: {
    de: 'Leere Karte',
    en: 'empty map',
    fr: 'carte vide',
  },
  zoomOut: {
    de: 'Heraus zoomen',
    en: 'Zoom out',
    fr: 'Zoom out',
  },
  zoomIn: {
    de: 'Hinein zoomen',
    en: 'Zoom in',
    fr: 'Zoom in',
  },
  zoomToScale: {
    de: 'massstabsgetreu vergrössern',
    en: 'zoom to scale',
    fr: "zoomer à l'échelle",
  },
  myLocation: {
    de: 'Aktuelle position',
    en: 'Current location',
    fr: 'Emplacement actuel',
  },
  defineCoordinates: {
    de: 'Koordinaten definieren',
    en: 'Define coordinates',
    fr: 'Définir les coordonnées',
  },
  coordinates: {
    de: 'Koordinaten',
    en: 'Coordinates',
    fr: 'Coordonnées',
  },
  csvLocation: {
    de: 'Koordinaten',
    en: 'Coordinates',
    fr: 'Coordonnées',
  },
  csvCentroid: {
    de: 'Koordinaten Zentrum',
    en: 'Centroid',
    fr: 'Centre coordonnées',
  },
  scale: {
    de: 'Massstab',
    en: 'scale',
    fr: 'échelle',
  },
  scaleToUse: {
    de: 'Massstab (1:xxx)',
    en: 'scale (1:xxx)',
    fr: 'échelle (1:xxx)',
  },
  openStreetMap: {
    de: 'OpenStreetMap',
    en: 'OpenStreetMap',
    fr: 'OpenStreetMap',
  },
  geoAdminSwissImage: {
    de: 'SWISSIMAGE',
    en: 'SWISSIMAGE',
    fr: 'SWISSIMAGE',
  },
  geoAdminPixel: {
    de: 'Pixelkarte farbig',
    en: 'Colored pixel map',
    fr: 'Carte pixelisée en couleur',
  },
  geoAdminPixelBW: {
    de: 'Pixelkarte grau',
    en: 'Gray pixel map',
    fr: 'Carte pixelisée en gris',
  },
  noBaseMap: {
    de: 'keine Basiskarte',
    en: 'no base Map',
    fr: 'pas de carte de base',
  },
  addNewLayer: {
    de: 'Neue Ebene erstellen',
    en: 'Create new layer',
    fr: 'Créer un nouveau couche',
  },
  layerSource: {
    de: 'Ebenen Quelle',
    en: 'Layer Source',
    fr: 'Source du couche',
  },
  layerSourceUrl: {
    de: 'Quell-URL',
    en: 'source URL',
    fr: 'URL source',
  },
  mapLayerType: {
    de: 'Ebenentyp',
    en: 'Layer type',
    fr: 'Type de calque',
  },
  globalMapLayers: {
    de: 'Geteilte Ebenen',
    en: 'Shared layers',
    fr: 'Couche partagées',
  },
  polygon: {
    de: 'Polygon',
    en: 'Polygon',
    fr: 'Polygone',
  },
  point: {
    de: 'Punkt',
    en: 'Point',
    fr: 'Point',
  },
  circle: {
    de: 'Kreis',
    en: 'Circle',
    fr: 'Cercle',
  },
  line: {
    de: 'Linie',
    en: 'Line',
    fr: 'Ligne',
  },
  freehand: {
    de: 'Freihand',
    en: 'Draw free',
    fr: 'Dessiner libre',
  },
  drawing: {
    de: 'Zeichnung',
    en: 'Drawing',
    fr: 'Dessin',
  },
  currentDrawing: {
    de: 'Aktuelle Zeichnung',
    en: 'Current drawing',
    fr: 'Dessin actuel',
  },
  drawMode: {
    de: 'Zeichnungsmodus',
    en: 'Drawing mode',
    fr: 'Mode de dessin',
  },
  expertView: {
    de: 'Experten Ansicht',
    en: 'Expert view',
    fr: "Point de vue d'expert",
  },
  defaultView: {
    de: 'Standard Ansicht',
    en: 'Default view',
    fr: 'Vue générale',
  },
  selectedFeature: {
    de: 'Ausgewähltes Feature',
    en: 'Selected Feature',
    fr: 'Fonctionnalité sélectionnée',
  },
  removeFeatureFromMapConfirm: {
    de: 'Möchten Sie dieses Element wirklich von der Karte entfernen?',
    en: 'Do you really want to remove this element from the map?',
    fr: 'Souhaitez-vous vraiment supprimer cet élément de la carte ?',
  },
  generalFilters: {
    de: 'Globale Filter',
    en: 'Global filters',
    fr: 'Filtre globale',
  },
  showAllElements: {
    de: 'Alle Elemente anzeigen',
    en: 'Show all elements',
    fr: 'Afficher tout',
  },
  hideAllElements: {
    de: 'Alle Elemente verstecken',
    en: 'Hide all elements',
    fr: 'Tout cacher',
  },
  drawnSymbols: {
    de: 'Gezeichnete Signaturen',
    en: 'Drawn symbols',
    fr: 'Symboles dessinés',
  },
  zoomToAll: {
    de: 'Auf alle gezeichnete Signaturen zoomen',
    en: 'Zoom to all drawn signatures',
    fr: 'Zoomez sur toutes les signatures dessinées',
  },
  highlightAll: {
    de: 'Alle gezeichnete Signaturen hervorheben',
    en: 'Highlight all drawn symbols',
    fr: 'Surlignez tous les symboles dessinés',
  },
  reset: {
    de: 'Zurücksetzen',
    en: 'Reset',
    fr: 'Réinitialiser',
  },
  csvSearchFor: {
    de: 'Suche nach ...',
    en: 'Search for ...',
    fr: 'Chercher ...',
  },
  csvID: {
    de: 'ID',
    en: 'ID',
    fr: 'ID',
  },
  csvDate: {
    de: 'Erstelldatum',
    en: 'Date created',
    fr: 'Date de creation',
  },
  csvGroup: {
    de: 'Gruppe',
    en: 'Group',
    fr: 'Groupe',
  },
  csvSignatur: {
    de: 'Signatur',
    en: 'Sign',
    fr: 'Signature',
  },
  csvSize: {
    de: 'Grösse',
    en: 'Size',
    fr: 'Dimension',
  },
  csvReportNumber: {
    de: 'Meldenummer',
    en: 'Report number',
    fr: 'Numéro de rapport',
  },
  csvLabel: {
    de: 'Bezeichnung',
    en: 'Label',
    fr: 'Désignation',
  },
  csvDescription: {
    de: 'Beschreibung',
    en: 'Description',
    fr: 'Description',
  },
  protocolExport: {
    de: 'ProtokollExport',
    en: 'ProtocolExport',
    fr: 'ExportationProtocole',
  },
};